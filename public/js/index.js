...
<ul class="node-list">
  <li v-for="(statement, idx) in node.statements" v-if="idx === 0 || expanded" @contextmenu="referenceStatement" :class="itemClasses[idx]">
    <i :class="{'fa': true, 'fa-check': true, 'valid-mark': true, 'closed-mark': !node.isValid(branches, idx) && node.isClosed(branches)}" v-if="node.isValid(branches, idx) || node.isClosed(branches)"></i>
