    ...
    isValid() : boolean {
        ...
        if (TruthTree.TERMINATORS.includes(this.statement)) {
            ...
            for (const nodeId of this.decomposition) {
                decomposedStmts.push(
                    this.tree.parseNodeStatement(nodeId)
                );
            }
...
class TruthTree {
    ...
    static readonly TERMINATORS = [TruthTree.OPEN_TERMINATOR, TruthTree.CLOSED_TERMINATOR];
